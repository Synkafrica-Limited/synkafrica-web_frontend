...
export default function OrdersPage() {
  ...
  const { bookings, loading, error, refetch } = useVendorBookings(token, {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F6]">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-gray-600">Location:</span><span className="font-medium text-gray-900">{selectedOrder.pickupLocation || selectedOrder.listing?.location || 'N/A'}</span></div>
                </div>
              </div>
              <div className="bg-primary-50 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <h4 className="font-semibold text-gray-900 flex items-center gap-2"><DollarSign className="w-5 h-5 text-primary-600" />Total Amount</h4>
      ...
      <div className="bg-white rounded-xl shadow-sm border mx-5 border-gray-200 p-4 mb-6 mt-4">
          ...
          {filteredBookings.map((booking) => {
            ...
            return (
              <div key={booking.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 hover:shadow-md transition-shadow">
                <div className="flex flex-col lg:flex-row lg:items-center mx-5 gap-4">
                  <div className="flex-1 space-y-3 ">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="font-semibold text-lg text-gray-900">{booking.listing?.title || 'Service'}</h3>
                        <p className="text-sm text-gray-600 mt-1">Order ID: <span className="font-medium">{booking.id}</span></p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                      <div className="flex items-center gap-2 text-gray-600"><User className="w-4 h-4 shrink-0" /><span className="truncate">{booking.customer?.name || 'N/A'}</span></div>
                      <div className="flex items-center gap-2 text-gray-600"><Calendar className="w-4 h-4 shrink-0" /><span>{formatDate(booking.startDate)}</span></div>
                      <div className="flex items-center gap-2 text-gray-600"><Clock className="w-4 h-4 shrink-0" /><span>{booking.duration || 'N/A'}</span></div>
                      <div className="flex items-center gap-2 text-primary-600 font-semibold"><DollarSign className="w-4 h-4 shrink-0" /><span>{formatCurrency(booking.totalAmount)}</span></div>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-500"><Clock className="w-3 h-3" />Created: {formatDate(booking.createdAt)}</div>
                  </div>
                  <div className="flex lg:flex-col gap-2">
                    <button onClick={() => viewDetails(booking)} className="flex-1 lg:flex-none px-4 py-2 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors text-sm flex items-center justify-center gap-2"><Eye className="w-4 h-4" />View Details</button>
                    {booking.status === "pending" && (
                      <>
                        <button onClick={() => handleAccept(booking)} className="flex-1 lg:flex-none px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition-colors text-sm flex items-center justify-center gap-2"><CheckCircle className="w-4 h-4" />Accept</button>
                        <button onClick={() => handleDecline(booking)} className="flex-1 lg:flex-none px-4 py-2 border border-red-300 text-red-700 rounded-lg font-medium hover:bg-red-50 transition-colors text-sm flex items-center justify-center gap-2"><XCircle className="w-4 h-4" />Decline</button>
                      </>
                    )}
                  </div>
                </div>
