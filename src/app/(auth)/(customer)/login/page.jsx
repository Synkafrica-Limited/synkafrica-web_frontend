...
export default function CustomerSignInScreen() {
  ...
  // Redirect if already logged in (session hook handles this, but double-check)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (accessToken) {
      ...
      // Store OAuth tokens with consistent naming
      if (finalRemember) {
  ...
  if (sessionLoading) {
    return (
      <AuthLayout title="Sign in" subtitle="Checking authentication...">
  ...
  if (isLoggedIn) {
    return (
      <AuthLayout title="Sign in" subtitle="Redirecting to dashboard...">
        <div className="flex justify-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500"></div>
        </div>
      </AuthLayout>
