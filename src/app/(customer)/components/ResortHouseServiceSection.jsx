...
const ResortHouseService = () => {
  ...
  const [resortCategories, setResortCategories] = useState([]);

  useEffect(() => {
    const fetchResorts = async () => {
      try {
        setLoading(true);
        const response = await getListings({ serviceType: 'RESORT' });
        ...
        apiListings = apiListings.filter(listing => listing.category === 'RESORT');

        // Transform data
        const transformedResorts = apiListings.map(listing => {
          // Image handling
          ...
          return {
            ...
            image: imageUrl,
            name: listing.title || listing.name || 'Luxury Resort',
            price: listing.pricing?.basePrice || listing.basePrice || 0,
            location: locationStr,
            category: listing.resortType || listing.category || 'Luxury'
          };
        });
        ...
        const categories = [...new Set(transformedResorts.map(r => r.category))].filter(Boolean).sort();
        setResortCategories(categories);

      } catch (err) {
        console.error("Failed to fetch resort listings:", err);
  ...
  if (error) {
    return <div className="py-20 text-center text-red-500">Failed to load resort options. Please try again later.</div>;
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredResorts.length > 0 ? (
            ...
            <div className="col-span-full text-center text-gray-500 py-10">
              No resorts found{activeCategory !== "All" && <> for <span className="font-semibold">{activeCategory}</span></>}.
            </div>
          )}
        </div>
      </div>
