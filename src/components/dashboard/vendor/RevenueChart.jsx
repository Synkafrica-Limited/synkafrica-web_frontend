...
import fallback from "@/lib/fallbackData/dashboard/revenueSeries.json";

/**
 * Revenue Chart Component
 * Displays revenue bar chart data with year selector (SVG)
 */
export default function RevenueChart({ series }) {
  ...
  const bars = data.map((d, i) => ({ ...d, height: (d.value / maxValue) * 100, index: i }));

  // compute bar sizing: use a narrower bar width and equal gaps
  const barBase = 100 / bars.length;
  const barWidthPercent = Math.max(barBase * 0.38, 3);
  ...
  return (
    <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
      ...
      <div className="relative h-48 sm:h-64">
        <div className="absolute left-0 top-0 bottom-0 w-12 sm:w-20 flex flex-col justify-between text-[10px] sm:text-xs text-gray-500">
        ...
        <div className="absolute left-12 sm:left-20 right-0 top-0 bottom-6 sm:bottom-8 flex items-end gap-1 sm:gap-2">
